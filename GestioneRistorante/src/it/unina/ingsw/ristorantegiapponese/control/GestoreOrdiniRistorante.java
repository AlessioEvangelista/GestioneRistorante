...
public class GestoreOrdiniRistorante {
	...
	public void creaComandaSenzaBevande(Conto conto, Piatto [] piatti, int [] qtaPiatti) {
		
		Comanda comanda = new Comanda(piatti, qtaPiatti);
		
		conto.creaComanda(comanda);
	}
	
	public void creaComandaSenzaPiatti(Conto conto, Bevanda[] bevande, int [] qtaBevande) {
		
		Comanda comanda = new Comanda(bevande, qtaBevande);
	...
	public int visualizzaConto(Conto conto) {
		...
		System.out.println("Totale conto: " + conto.calcolaPrezzo() + "$");
		return conto.calcolaPrezzo();
	}
